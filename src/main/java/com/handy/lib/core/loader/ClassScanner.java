...
public class ClassScanner implements Serializable {
     ...
     * @return 类集合
     */
    protected static Set<Class<?>> scanPackageByAnnotation(String packageName, final Class<? extends Annotation> annotationClass) {
     ...
     * @return 类集合
     */
    protected static Set<Class<?>> scanPackageBySuper(String packageName, final Class<?> superClass) {
     ...
     * @return 类集合
     */
    protected static Set<Class<?>> scanPackage(String packageName, Filter<Class<?>> classFilter) {
     ...
     */
    protected ClassScanner(String packageName, Filter<Class<?>> classFilter) {
     ...
     * @param charset     编码
     */
    protected ClassScanner(String packageName, Filter<Class<?>> classFilter, Charset charset) {
        packageName = ClassStrUtil.nullToEmpty(packageName);
        this.packageName = packageName;
        this.packageNameWithDot = ClassStrUtil.addSuffixIfNot(packageName, ".");
        this.packageDirName = packageName.replace('.', File.separatorChar);
        this.packagePath = packageName.replace('.', CharUtil.SLASH);
    ...
    private void addIfAccept(String className) {
        if (ClassStrUtil.isBlank(className)) {
            return;
        }
    ...
    private String subPathBeforePackage(File file) {
        ...
        if (ClassStrUtil.isNotEmpty(this.packageDirName)) {
            filePath = ClassStrUtil.subBefore(filePath, this.packageDirName, true);
        }
        return ClassStrUtil.addSuffixIfNot(filePath, File.separator);
    }
    // --------------------------------------------------------------------------------------------------- Private method end
}
