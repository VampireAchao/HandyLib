...
public class URLDecoder implements Serializable {
     ...
     * @return 解码后的字符串
     */
    protected static String decode(String str, Charset charset) {
     ...
     * @return 解码后的字符串
     */
    protected static String decode(String str, Charset charset, boolean isPlusToSpace) {
        return ClassStrUtil.str(decode(ClassStrUtil.bytes(str, charset), isPlusToSpace), charset);
    }
     ...
     * @since 5.6.3
     */
    protected static byte[] decode(byte[] bytes, boolean isPlusToSpace) {
        if (bytes == null) {
            return null;
        }
